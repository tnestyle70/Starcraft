...
CRoboticsSupportBay::CRoboticsSupportBay()
{
}
...
void CRoboticsSupportBay::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 160.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"Robotics_Support_Bay");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"Robotics_Support_Bay";
...
void CRoboticsSupportBay::SetBuildingData()
{
	m_eType = eBuildingType::ROBOTICS_SUPPORT_BAY;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
int CRoboticsSupportBay::Update()
{
...
void CRoboticsSupportBay::Render(HDC hDC)
{
...
void CRoboticsSupportBay::Release()
{
}

void CRoboticsSupportBay::RenderSlot(HDC hDC, int slotIndex)
{
}

int CRoboticsSupportBay::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CRoboticsSupportBay::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
	...
	if (m_eState == eBuildingState::CONSTRUCTING)
		return;
	//5번 슬롯 차원 관문 업그레이드
	outSlot[5].commandID = eCommandID::GATEWAY;
	...
	//7번 슬롯 드라군 블링크 업그레이드
	outSlot[7].commandID = eCommandID::BLINK;
...
void CRoboticsSupportBay::UpdateHotKeys()
{
...
bool CRoboticsSupportBay::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	...
	switch (command)
	{
	case eCommandID::GATEWAY: //차원 관문 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::GATEWAY, 1.f, 1.f, 50, 0 });
		return true;
	case eCommandID::RUSH: //차원 관문 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::RUSH, 1.f, 1.f, 50, 0 });
		return true;
	case eCommandID::BLINK: //차원 관문 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::BLINK, 1.f, 1.f, 50, 0 });
		return true;
...
void CRoboticsSupportBay::Rally()
{
...
void CRoboticsSupportBay::Landing()
{
	if (m_bStartLanding)
	{
		//랜딩 가능 검증
...
void CRoboticsSupportBay::BuildAddOn()
{
...
void CRoboticsSupportBay::UpdateProduction()
{
...
void CRoboticsSupportBay::ProductionComplete(eCommandID command)
{
	if (command == eCommandID::GATEWAY)
	{
		//사운드 재생
		CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/UpgradeComplete.wav", 1.f);
		//차원 관문 업그레이드 완료 
		CObjMgr::Get_Instance()->SetWarpGateReady(true);
	}
	if (command == eCommandID::RUSH)
	{
		//사운드 재생
		CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/UpgradeComplete.wav", 1.f);
		//질럿 돌진
	}
	if (command == eCommandID::BLINK)
	{
		//사운드 재생
		CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/UpgradeComplete.wav", 1.f);
		//드라군 점멸
...
void CRoboticsSupportBay::UseScanner()
{
	//사운드 재생
...
void CRoboticsSupportBay::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CRoboticsSupportBay::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CRoboticsSupportBay::BuildNuclearSilo()
{
