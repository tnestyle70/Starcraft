...
void CReavor::Render(HDC hDC)
{
    ...
    TCHAR szKey[256];
    wsprintf(szKey, L"Reavor_Move%d_Anim%d", dir16, m_tFrame.iStart);
...
void CReavor::Release()
{
}

void CReavor::UpdateHotKeys()
{
    CUnit::UpdateHotKeys();
}

bool CReavor::ExecuteCommand(eCommandID command, CommandContext& context)
{
    //먼저 부모 명령 실행
...
void CReavor::CommandCardSlot(vector<CommandSlot>& outSlot)
{
...
void CReavor::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"reaver/ptrdth00.wav", 1.f);
