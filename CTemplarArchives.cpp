...
void CTemplarArchives::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 160.f;
	m_tInfo.fCY = 192.f;
	lstrcpy(m_szGreenKey, L"Templar_Archives");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"Templar_Archives";
...
void CTemplarArchives::SetBuildingData()
{
	m_eType = eBuildingType::TEMPLAR_ARCHIVES;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CTemplarArchives::Render(HDC hDC)
{
...
void CTemplarArchives::Release()
{
}

void CTemplarArchives::RenderSlot(HDC hDC, int slotIndex)
{
}

int CTemplarArchives::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CTemplarArchives::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CTemplarArchives::UpdateHotKeys()
{
...
bool CTemplarArchives::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	...
	switch (command)
	{
	case eCommandID::STORM: // 스톰 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::STORM, 10.f, 10.f, 75, 75 });
		return true;
	case eCommandID::HALLUCINATION: //차원 관문 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::HALLUCINATION, 10.f, 10.f, 50, 50 });
		return true;
...
void CTemplarArchives::BuildAddOn()
{
...
void CTemplarArchives::UpdateProduction()
{
...
void CTemplarArchives::ProductionComplete(eCommandID command)
{
	if (command == eCommandID::STORM)
	{
		//사운드 재생
	...
	if (command == eCommandID::HALLUCINATION)
	{
		//사운드 재생
		CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/UpgradeComplete.wav", 1.f);
		//질럿 돌진
...
void CTemplarArchives::UseScanner()
{
	//사운드 재생
...
void CTemplarArchives::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CTemplarArchives::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CTemplarArchives::BuildNuclearSilo()
{
