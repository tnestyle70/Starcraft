...
void CAssimilator::SetBuildingData()
{
	m_eType = eBuildingType::ASSIMILATOR;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CAssimilator::Render(HDC hDC)
{
...
void CAssimilator::Release()
{
}

void CAssimilator::RenderSlot(HDC hDC, int slotIndex)
{
}

int CAssimilator::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CAssimilator::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CAssimilator::UpdateHotKeys()
{
...
bool CAssimilator::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CAssimilator::UpdateProduction()
{
...
void CAssimilator::ProductionComplete(eCommandID command)
{
...
void CAssimilator::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CAssimilator::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CAssimilator::BuildNuclearSilo()
{
