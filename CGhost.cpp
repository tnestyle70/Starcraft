...
void CGhost::Initialize()
{
    ...
    //공격 애니메이션 프레임 - 9~12 col까지가 공격 애니메이션 프레임
    m_iAttackFrameStart = 9;
...
void CGhost::FireBullet()
{
    ...
    if (dist <= m_fAttackRange)
    {
        ...
        //사운드 재생
        CSoundMgr::Get_Instance()->PlayEffect(L"Ghost/GhostAttack.wav", 0.6f);
        //이펙트 생성
        m_fAttackSpeed = 2.5f;
        //타겟 이펙트 생성
        ...
        if (pEffectObj)
        {
            pEffectObj->Set_Effect(L"GHOST_SHOT_EFFECT",
                10, 20, 20, eEffectType::ROW_BASE, RGB(0, 255, 0));
        }
...
bool CGhost::ExecuteCommand(eCommandID command, CommandContext& context)
{
...
void CGhost::CommandCardSlot(vector<CommandSlot>& outSlot)
{
    CUnit::CommandCardSlot(outSlot);

    bool bMissileReady = CObjMgr::Get_Instance()->IsMissileReady();

    //7번 : 클로킹
    outSlot[6].commandID = eCommandID::CLOCKING;
    outSlot[6].iconKey = TEXT("ICON_STEAMPACK");
    outSlot[6].hotkey = 'X';
    outSlot[6].clickable = true;
    outSlot[6].visible = true;
    //9번 : 핵
    outSlot[8].commandID = eCommandID::NUCLEAR;
    outSlot[8].iconKey = TEXT("ICON_STEAMPACK");
    outSlot[8].hotkey = 'V';
    outSlot[8].clickable = true;
    outSlot[8].visible = true;
    outSlot[8].lock = !bMissileReady;
}


void CGhost::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Ghost/GhostDeath1.wav", 1.f);
...
void CGhost::Clocking()
{
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Ghost/GhostClocking.wav", 0.7f);
}

void CGhost::NuclearLaunch()
{
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/NuclearLaunchedDetected.wav", 0.5f);
    //핵 발사 업데이트

    Vec2 worldMouse = CInputMgr::Get_Instance()->GetWorldMouse();
    // 투사체 생성
    CGhostBullet* pBullet = new CGhostBullet;
    pBullet->Initialize();
    pBullet->Set_Pos(worldMouse.fX, worldMouse.fY - 350.f); //타겟 기준 위쪽에서 생성
    pBullet->Set_Target_Pos(worldMouse);
    pBullet->Set_Owner(this);
    CObjMgr::Get_Instance()->Add_Object(OBJID::OBJ_PROJECTILE, pBullet);
}

void CGhost::NuclearLaunchEnemy(Vec2 worldMouse)
{
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Advisor/NuclearLaunchedDetected.wav", 0.5f);
    // 투사체 생성
...
CObj* CGhost::FindNearestEnemyAI(float searchRadius)
{
