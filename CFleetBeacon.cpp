...
void CFleetBeacon::SetBuildingData()
{
	m_eType = eBuildingType::FLEET_BEACON;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CFleetBeacon::Render(HDC hDC)
{
...
void CFleetBeacon::Release()
{
}

void CFleetBeacon::RenderSlot(HDC hDC, int slotIndex)
{
}

int CFleetBeacon::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CFleetBeacon::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CFleetBeacon::UpdateHotKeys()
{
...
bool CFleetBeacon::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CFleetBeacon::UpdateProduction()
{
...
void CFleetBeacon::ProductionComplete(eCommandID command)
{
...
void CFleetBeacon::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CFleetBeacon::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CFleetBeacon::BuildNuclearSilo()
{
