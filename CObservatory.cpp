...
void CObservatory::SetBuildingData()
{
	m_eType = eBuildingType::OBSERVATORY;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CObservatory::Render(HDC hDC)
{
...
void CObservatory::Release()
{
}

void CObservatory::RenderSlot(HDC hDC, int slotIndex)
{
}

int CObservatory::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CObservatory::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CObservatory::UpdateHotKeys()
{
...
bool CObservatory::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CObservatory::UpdateProduction()
{
...
void CObservatory::ProductionComplete(eCommandID command)
{
...
void CObservatory::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CObservatory::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CObservatory::BuildNuclearSilo()
{
