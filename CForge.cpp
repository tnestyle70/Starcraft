...
void CForge::SetBuildingData()
{
	m_eType = eBuildingType::FORGE;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CForge::RenderSlot(HDC hDC, int slotIndex)
{
}

int CForge::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CForge::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
bool CForge::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CForge::ProductionComplete(eCommandID command)
{
...
void CForge::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
