...
void CMutalisk::Initialize()
{
    ...
    m_fAttackInterval = 0.5f; //진짜 공격 속도 타이머!
...
int CMutalisk::Update()
{
    ...
    case eUnitState::ATTACK:
        if (m_bAttack)
        {
            // 방향 설정
            m_tFrame.iFrame = DirTo16WayIndex(m_vDir);
            // 애니메이션 진행, 뮤탈리스크는 기본 Move 애니메이션 재생
            if (now - m_tFrame.dwTime >= m_tFrame.dwSpeed)
            {
                m_tFrame.iStart++;
                if (m_tFrame.iStart > m_tFrame.iEnd)
...
void CMutalisk::Late_Update()
{
}

void CMutalisk::Render(HDC hDC)
{
    // 전장의 안개
    CUnit::Render(hDC);
    ...
    case eUnitState::IDLE:
    case eUnitState::MOVE:
        wsprintf(szKey, L"Mutalisk_Move%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
    case eUnitState::ATTACK:
        wsprintf(szKey, L"Mutalisk_Move%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
...
void CMutalisk::Release()
{
}

void CMutalisk::UpdateAI()
{
    //주변 적 찾기
...
CObj* CMutalisk::FindNearestEnemyAI(float searchRadius)
{
...
void CMutalisk::FireBullet()
{
    ...
    if (dist <= m_fAttackRange)
    {
        ...
        //공격 사운드 재생
        CSoundMgr::Get_Instance()->PlayEffect(L"Mutalisk/MutaliskBirth.wav", 0.3f);

        //현재 방향 가져오기 (16방향)
        int iCurrentDir = m_tFrame.iFrame;

        // 투사체 생성
        CMutaliskBullet* pBullet = new CMutaliskBullet;
        pBullet->Initialize();
...
void CMutalisk::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Mutalisk/MutaliskDeath.wav", 0.5f);
