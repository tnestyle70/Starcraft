...
void CSpire::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 128.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"SPIRE");
...
void CSpire::PlayCompleteSound()
{
	CSoundMgr::Get_Instance()->PlayEffect(L"Overload/zovDth00.wav", 0.5f);
}

void CSpire::SetBuildingData()
{
	m_eType = eBuildingType::SPIRE;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CSpire::RenderSlot(HDC hDC, int slotIndex)
{
}

int CSpire::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CSpire::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
bool CSpire::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	if (m_eState != eBuildingState::CONSTRUCT)
		return false;
...
void CSpire::ProductionComplete(eCommandID command)
{
