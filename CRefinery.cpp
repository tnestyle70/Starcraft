...
void CRefinery::Initialize()
{
	...
	m_tInfo.fCX = 192.f;
	m_tInfo.fCY = 192.f;
	lstrcpy(m_szGreenKey, L"REFINERY_GREEN");
	lstrcpy(m_szRedKey, L"REFINERY_RED");
	m_pFrameKey = L"Refinery";
...
{
	m_eType = eBuildingType::REFINERY;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CRefinery::RenderSlot(HDC hDC, int slotIndex)
{
}

int CRefinery::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CRefinery::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
bool CRefinery::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CRefinery::UpdateProduction()
{
...
void CRefinery::ConstructComplete(eCommandID command)
{
	//인구수 올리기
	/*
