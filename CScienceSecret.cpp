...
void CScienceSecret::Initialize()
{
...
void CScienceSecret::SetBuildingData()
{
	m_eType = eBuildingType::SCIENCE_SECRET;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CScienceSecret::Render(HDC hDC)
{
...
void CScienceSecret::Release()
{
}

void CScienceSecret::RenderSlot(HDC hDC, int slotIndex)
{
}

int CScienceSecret::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CScienceSecret::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CScienceSecret::UpdateHotKeys()
{
...
bool CScienceSecret::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CScienceSecret::UpdateAnimation()
{
	if (m_bProducing)
	{
...
void CScienceSecret::UseScanner()
{
	//사운드 재생
...
void CScienceSecret::UpdateProduction()
{
...
void CScienceSecret::ConstructComplete(eCommandID command)
{
	//인구수 올리기
	/*
...
void CScienceSecret::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CScienceSecret::PlayCompleteSound()
{
