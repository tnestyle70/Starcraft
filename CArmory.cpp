...
void CArmory::Initialize()
{
	...
	m_tInfo.fCX = 160.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"ARMORY");
	lstrcpy(m_szRedKey, L"ARMORY");
	m_pFrameKey = L"ARMORY_ANIM";
...
void CArmory::SetBuildingData()
{
	m_eType = eBuildingType::ARMORY;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CArmory::RenderSlot(HDC hDC, int slotIndex)
{
}

int CArmory::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CArmory::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
	// 1. 부모 클래스의 공통 슬롯을 먼저 가져오기
	CBuilding::CommandCardSlot(outSlot);
	//0번 슬롯 vehicle attack upgrade
	outSlot[0].commandID = eCommandID::VEHICLE_ATTACK_UPGRADE;
	outSlot[0].iconKey = TEXT("ICON_ATTACK_UPGRADE");
	outSlot[0].hotkey = 'W';
	outSlot[0].clickable = true;
	outSlot[0].visible = true;
	//1번 슬롯 vehicle armor upgrade
	outSlot[1].commandID = eCommandID::VEHICLE_ARMOR_UPGRADE;
	outSlot[1].iconKey = TEXT("ICON_ARMOR_UPGRADE");
	outSlot[1].hotkey = 'E';
	outSlot[1].clickable = true;
	outSlot[1].visible = true;
	//2번 슬롯 ship attack upgrade
	outSlot[3].commandID = eCommandID::SHIP_ATTACK_UPGRADE;
	outSlot[3].iconKey = TEXT("ICON_ATTACK_UPGRADE");
	outSlot[3].hotkey = 'S';
	outSlot[3].clickable = true;
	outSlot[3].visible = true;
	//3번 슬롯 ship armor upgrade
	outSlot[4].commandID = eCommandID::SHIP_ARMOR_UPGRADE;
...
bool CArmory::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	if (m_eState != eBuildingState::CONSTRUCT)
		return false;
	...
	case eCommandID::VEHICLE_ATTACK_UPGRADE:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, false))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::VEHICLE_ATTACK_UPGRADE, 10.f, 10.f, 50, 0 });
		return true;
	case eCommandID::VEHICLE_ARMOR_UPGRADE:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, false))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::VEHICLE_ARMOR_UPGRADE, 10.f, 10.f, 50, 0 });
		return true;
	case eCommandID::SHIP_ATTACK_UPGRADE:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, false))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::SHIP_ATTACK_UPGRADE, 10.f, 10.f, 50, 0 });
		return true;
	case eCommandID::SHIP_ARMOR_UPGRADE:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, false))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::SHIP_ARMOR_UPGRADE, 10.f, 10.f, 50, 0 });
		return true;
	default:
		break;
	}
...
void CArmory::ProductionComplete(eCommandID command)
{
	if (command == eCommandID::VEHICLE_ATTACK_UPGRADE)
	{
		//업그레이드 매니저로 한 번에 관리하기
	}
	else if (command == eCommandID::VEHICLE_ARMOR_UPGRADE)
	{
		//업그레이드 매니저로 한 번에 관리하기
	}
	else if (command == eCommandID::SHIP_ARMOR_UPGRADE)
	{
		//업그레이드 매니저로 한 번에 관리하기
	}
	else if (command == eCommandID::SHIP_ARMOR_UPGRADE)
