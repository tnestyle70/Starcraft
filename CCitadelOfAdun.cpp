...
void CCitadelOfAdun::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 160.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"CitadelOfAdun");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"CitadelOfAdun";
...
void CCitadelOfAdun::SetBuildingData()
{
	m_eType = eBuildingType::CITADEL_OF_ADUN;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CCitadelOfAdun::Render(HDC hDC)
{
...
void CCitadelOfAdun::Release()
{
}

void CCitadelOfAdun::RenderSlot(HDC hDC, int slotIndex)
{
}

int CCitadelOfAdun::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CCitadelOfAdun::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CCitadelOfAdun::UpdateHotKeys()
{
...
bool CCitadelOfAdun::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	...
	switch (command)
	{
	case eCommandID::RUSH: //질럿 속도 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::RUSH, 10.f, 10.f, 50, 50 });
		return true;
	case eCommandID::BLINK: //드라군 점멸 업그레이드
		cost.mineral = 50;
		cost.gas = 50;
		cost.supply = 0;
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::BLINK, 10.f, 10.f, 50, 50 });
		return true;
...
void CCitadelOfAdun::BuildAddOn()
{
...
void CCitadelOfAdun::UpdateProduction()
{
...
void CCitadelOfAdun::ProductionComplete(eCommandID command)
{
...
void CCitadelOfAdun::UseScanner()
{
	//사운드 재생
...
void CCitadelOfAdun::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CCitadelOfAdun::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CCitadelOfAdun::BuildNuclearSilo()
{
