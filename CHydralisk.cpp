...
#include "CHydraliskBullet.h"
#include "CTimeMgr.h"

CHydralisk::CHydralisk() : 
    iAttackFrameStart(0)
    ,iAttackFrameEnd(7)
{
}
...
void CHydralisk::Initialize()
{
    ...
    // 공격 관련
    m_iAttackDamage = 10;
    m_fAttackRange = 160.f;

    m_fAttackInterval = 0.3f; //진짜 공격 속도 타이머!
...
void CHydralisk::Late_Update()
{
}

void CHydralisk::Render(HDC hDC)
{
    // 전장의 안개
    CUnit::Render(hDC);
    ...
    case eUnitState::IDLE:
    case eUnitState::MOVE:
        wsprintf(szKey, L"Hydralisk_Move%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
    case eUnitState::ATTACK:
        wsprintf(szKey, L"Hydralisk_Attack%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
    case eUnitState::DIE:
        break;
    default:
...
void CHydralisk::Release()
{
}

void CHydralisk::UpdateAI()
{
    //주변 적 찾기
...
CObj* CHydralisk::FindNearestEnemyAI(float searchRadius)
{
...
void CHydralisk::FireBullet()
{
    ...
    if (dist <= m_fAttackRange)
    {
        ...
        //공격 사운드 재생
        CSoundMgr::Get_Instance()->PlayEffect(L"Hydralisk/HydraliskAttack.wav", 0.3f);

        // 현재 방향 가져오기 (16방향)
        int iCurrentDir = m_tFrame.iFrame;

        // 투사체 생성
        CHydraliskBullet* pBullet = new CHydraliskBullet;
        pBullet->Initialize();
...
void CHydralisk::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Hydralisk/HydraliskDeath.wav", 0.5f);
