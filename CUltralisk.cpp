...
void CUltralisk::Initialize()
{
    ...
    m_fAttackInterval = 0.2f; //진짜 공격 속도 타이머!
    ...
    m_eType = eUnitType::ULTRALISK;

    // 프레임 정보 초기화
    m_tFrame.iFrame = 0;        // 현재 방향 (0~15)
    m_tFrame.iStart = 0;        // 현재 프레임 (0~7)
    m_tFrame.iEnd = 8;          // 마지막 프레임
    m_tFrame.iCol = 0;
    m_tFrame.dwTime = GetTickCount();
    m_tFrame.dwSpeed = 100;      // 빠른 애니메이션 (80ms)
}

int CUltralisk::Update()
{
...
void CUltralisk::Late_Update()
{
}

void CUltralisk::Render(HDC hDC)
{
    // 전장의 안개
    CUnit::Render(hDC);
    ...
    case eUnitState::IDLE:
    case eUnitState::MOVE:
        wsprintf(szKey, L"Ultralisk_Move%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
    case eUnitState::ATTACK:
        wsprintf(szKey, L"Ultralisk_Attack%d_Anim%d", m_tFrame.iFrame, m_tFrame.iStart);
        break;
    case eUnitState::DIE:
        break;
    default:
...
void CUltralisk::Release()
{
}

void CUltralisk::UpdateAI()
{
    //주변 적 찾기
...
CObj* CUltralisk::FindNearestEnemyAI(float searchRadius)
{
...
void CUltralisk::FireBullet()
{
    ...
    if (dist <= m_fAttackRange)
    {
        ...
        //공격 사운드 재생
        CSoundMgr::Get_Instance()->PlayEffect(L"Ultralisk/UltraliskSelect4.wav", 0.4f);
        //이펙트 생성
        m_fAttackSpeed = 2.5f;
        return;
    }
...
void CUltralisk::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Ultralisk/UltraiskDeath.wav", 0.5f);
