...
void CScout::Render(HDC hDC)
{
    ...
    case eUnitState::IDLE:
    case eUnitState::MOVE:
        wsprintf(szKey, L"Scout_Move_%d", dir16);
        break;
    case eUnitState::ATTACK:
        wsprintf(szKey, L"Scout_Attack_%d", dir16);
        break;
    case eUnitState::DIE:
        break;
    default:
    ...
    //wsprintf(szKey, L"Scout_Move_%d", fileIndex);
...
void CScout::UpdateHotKeys()
{
    CUnit::UpdateHotKeys();
}

bool CScout::ExecuteCommand(eCommandID command, CommandContext& context)
{
    //먼저 부모 명령 실행
...
void CScout::CommandCardSlot(vector<CommandSlot>& outSlot)
{
...
void CScout::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"Scout/PScDth00.wav", 1.f);
...
void CScout::FireBullet()
{
    ...
    if (dist <= m_fAttackRange)
    {
        ...
        //사운드 재생
        CSoundMgr::Get_Instance()->PlayEffect(L"Scout/blastgn2.wav", 0.5f);
        //이펙트 생성
        CObj* pEffect = CAbstractFactory<CEffect>::Create(
        ...
        if (pEffectObj)
        {
            pEffectObj->Set_Effect(L"Scout_GroundEffect_",
                8, 40, 40, eEffectType::COL_BASE, RGB(0, 0, 0), 0.05f, false,
                eEffectImageType::PNG);
        }
        CObjMgr::Get_Instance()->Add_Object(OBJ_PROJECTILE, pEffect);
...
CObj* CScout::FindNearestEnemyAI(float searchRadius)
{
