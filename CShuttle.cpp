...
void CShuttle::Render(HDC hDC)
{
    ...
    TCHAR szKey[256];
    wsprintf(szKey, L"Shuttle_Move_%d", fileIndex);
...
void CShuttle::Release()
{
}

void CShuttle::UpdateHotKeys()
{
    CUnit::UpdateHotKeys();
}

bool CShuttle::ExecuteCommand(eCommandID command, CommandContext& context)
{
    //먼저 부모 명령 실행
...
void CShuttle::CommandCardSlot(vector<CommandSlot>& outSlot)
{
    CUnit::CommandCardSlot(outSlot);
    //7번 : 차원분광기 사용
    outSlot[6].commandID = eCommandID::PYLON;
    ...
    //8번 : Loading 사용
    outSlot[7].commandID = eCommandID::LOADING;
    ...
    //9번 : Landing 사용
    outSlot[8].commandID = eCommandID::LANDING;
...
void CShuttle::UpdateDead()
{
    ...
    //이펙트와 사운드 재생
    CSoundMgr::Get_Instance()->PlayEffect(L"shuttle/pshdth00.wav", 1.f);
...
void CShuttle::LoadUnit(CUnit* pUnit)
{
...
void CShuttle::UnloadUnit()
{
    ...
    for (auto& pUnit : m_vecLoadUnits)
    {
        ...
        pUnit->Set_Pos(m_tInfo.fX + offset, m_tInfo.fY + 50.f);
        offset += 20.f;
    }
    m_vecLoadUnits.clear();
}

void CShuttle::ShowLoadUnits()
{
...
void CShuttle::RenderPower(HDC hDC)
{
    if (!m_bPower)
        return;
...
CObj* CShuttle::FindNearestEnemyAI(float searchRadius)
{
