...
void CSpawningPool::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 96.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"SPAWNING_POOL");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"SPAWNING_POOL";
...
void CSpawningPool::PlayCompleteSound()
{
	CSoundMgr::Get_Instance()->PlayEffect(L"Overload/zovDth00.wav", 0.5f);
}

void CSpawningPool::SetBuildingData()
{
	m_eType = eBuildingType::SPAWNING_POOL;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CSpawningPool::Render(HDC hDC)
{
...
void CSpawningPool::Release()
{
}

void CSpawningPool::RenderSlot(HDC hDC, int slotIndex)
{
}

int CSpawningPool::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CSpawningPool::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CSpawningPool::UpdateHotKeys()
{
...
bool CSpawningPool::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	if (m_eState != eBuildingState::CONSTRUCT)
		return false;
...
void CSpawningPool::UpdateProduction()
{
...
void CSpawningPool::ProductionComplete(eCommandID command)
{
...
void CSpawningPool::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CSpawningPool::SpawnZergUnits()
{
