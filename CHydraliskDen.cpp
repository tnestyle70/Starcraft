...
void CHydraliskDen::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 160.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"HYDRALISK_DEN");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"HYDRALISK_DEN";
...
void CHydraliskDen::SetBuildingData()
{
	m_eType = eBuildingType::HYDRALISK_DEN;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CHydraliskDen::Render(HDC hDC)
{
...
void CHydraliskDen::Release()
{
}

void CHydraliskDen::RenderSlot(HDC hDC, int slotIndex)
{
}

int CHydraliskDen::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CHydraliskDen::PlayCompleteSound()
{
	CSoundMgr::Get_Instance()->PlayEffect(L"Overload/zovDth00.wav", 0.5f);
}

void CHydraliskDen::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CHydraliskDen::UpdateHotKeys()
{
...
bool CHydraliskDen::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	if (m_eState != eBuildingState::CONSTRUCT)
		return false;
...
void CHydraliskDen::UpdateProduction()
{
...
void CHydraliskDen::ProductionComplete(eCommandID command)
{
...
void CHydraliskDen::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CHydraliskDen::SpawnZergUnits()
{
