...
void CStarportAddOn::Initialize()
{
	...
	m_tInfo.fCX = 128.f;
	m_tInfo.fCY = 64.f;
	lstrcpy(m_szGreenKey, L"STARPORT_ADDON");
	lstrcpy(m_szRedKey, L"STARPORT_ADDON");
	m_pFrameKey = L"STARPORT_ADDON_ANIM";
...
void CStarportAddOn::SetBuildingData()
{
	m_eType = eBuildingType::STARPORT_ADDON;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CStarportAddOn::RenderSlot(HDC hDC, int slotIndex)
{
}

int CStarportAddOn::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CStarportAddOn::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
bool CStarportAddOn::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
...
void CStarportAddOn::UpdateHotKeys()
{
...
void CStarportAddOn::UpdateAnimation()
{
	if (m_bProducing)
	{
...
void CStarportAddOn::UpdateProduction()
{
...
void CStarportAddOn::ConstructComplete(eCommandID command)
{
	//인구수 올리기
	/*
...
void CStarportAddOn::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CStarportAddOn::PlayCompleteSound()
{
