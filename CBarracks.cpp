...
void CBarracks::UpdateAnimation()
{
...
void CBarracks::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
	...
	bool bAcademyBuilt = CObjMgr::Get_Instance()->IsAcademyBuilt();
	...
	//2번 슬롯 FIREBAT 생산
	outSlot[2].commandID = eCommandID::FIREBAT;
	outSlot[2].iconKey = TEXT("ICON_FIREBAT");
	outSlot[2].hotkey = 'R';
	outSlot[2].clickable = true;
	outSlot[2].visible = true;
	outSlot[2].lock = !bAcademyBuilt;
	//3번 슬롯 GHOST 생산
	outSlot[3].commandID = eCommandID::GHOST;
	outSlot[3].iconKey = TEXT("ICON_GHOST");
	outSlot[3].hotkey = 'S';
	outSlot[3].clickable = true;
	outSlot[3].visible = true;
	outSlot[3].lock = !bAcademyBuilt;
	//4번 슬롯 LIFT
...
bool CBarracks::ExecuteCommand(eCommandID command, CommandContext& context)
{
	...
	case eCommandID::MARINE:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::MARINE, 1.f, 1.f, 50, 0, 1 });
		return true;
	case eCommandID::MEDIC:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::MEDIC, 1.f, 1.f, 50, 0, 1 });
		return true;
	case eCommandID::FIREBAT:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::FIREBAT, 1.f, 1.f, 50, 0, 1 });
		return true;
	case eCommandID::GHOST:
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::GHOST, 1.f, 1.f, 50, 0, 1 });
		return true;
...
void CBarracks::ProductionComplete(eCommandID command)
{
	...
	{
	case eCommandID::MARINE:
		pUnit = CAbstractFactory<CMarine>::Create(pos.fX, pos.fY);
		CSoundMgr::Get_Instance()->PlayEffect(L"Marine/MarineBirth.wav", 0.3f);
		break;
	case eCommandID::MEDIC:
		pUnit = CAbstractFactory<CMedic>::Create(pos.fX, pos.fY);
		CSoundMgr::Get_Instance()->PlayEffect(L"Medic/MedicBirth.wav", 0.3f);
		break;
	case eCommandID::FIREBAT:
		pUnit = CAbstractFactory<CFirebat>::Create(pos.fX, pos.fY);
		CSoundMgr::Get_Instance()->PlayEffect(L"FireBat/FireBatBirth.wav", 0.3f);
		break;
	case eCommandID::GHOST:
		pUnit = CAbstractFactory<CGhost>::Create(pos.fX, pos.fY);
		CSoundMgr::Get_Instance()->PlayEffect(L"Ghost/GhostBirth.wav", 0.3f);
		break;
	default:
		break;
	}
...
void CBarracks::SpawnTerranEnemy()
{
