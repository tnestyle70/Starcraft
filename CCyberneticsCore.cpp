...
void CCyberneticsCore::Initialize()
{
	CBuilding::Initialize();
	m_tInfo.fCX = 96.f;
	m_tInfo.fCY = 128.f;
	lstrcpy(m_szGreenKey, L"Cybernetics_Core");
	lstrcpy(m_szRedKey, L"COMMANDCENTER_RED");
	lstrcpy(m_szConstructKey, L"COMMANDCENTER_CONSTRUCT");
	m_pFrameKey = L"Cybernetics_Core";
...
void CCyberneticsCore::SetBuildingData()
{
	m_eType = eBuildingType::CYBERNETICS_CORE;
	//비용, 스탯
	m_tCost.mineral = 100;
	m_tCost.gas = 0;
	m_tCost.supply = 0;
...
void CCyberneticsCore::Render(HDC hDC)
{
...
void CCyberneticsCore::Release()
{
}

void CCyberneticsCore::RenderSlot(HDC hDC, int slotIndex)
{
}

int CCyberneticsCore::GetIconIndex(eCommandID eCommand)
{
	return 0;
}

void CCyberneticsCore::CommandCardSlot(std::vector<CommandSlot>& outSlot)
{
...
void CCyberneticsCore::UpdateHotKeys()
{
...
bool CCyberneticsCore::ExecuteCommand(eCommandID command, CommandContext& context)
{
	//건물이 다 지어진 이후에 건설 가능
	...
	case eCommandID::GATEWAY: //차원 관문 업그레이드
		...
		if (!CResourceMgr::Get_Instance()->TrySpend(cost, true))
			return false;
		//생산 시간
		m_queue.push_back({ eCommandID::GATEWAY, 10.f, 10.f, 50, 50 });
		return true;
...
void CCyberneticsCore::BuildAddOn()
{
...
void CCyberneticsCore::UpdateProduction()
{
...
void CCyberneticsCore::ProductionComplete(eCommandID command)
{
...
void CCyberneticsCore::UseScanner()
{
	//사운드 재생
...
void CCyberneticsCore::Destroy()
{
	m_eState = eBuildingState::DESTROY;
}

void CCyberneticsCore::BuildCombatStation()
{
	CBuilding* pAddOn = CBuildingFactory::Create(eBuildingType::COMBAT_STATION);
	//factory 기준 오른쪽에 배치
	pAddOn->Set_Pos(m_tInfo.fX + 80.f, m_tInfo.fY + 20.f);
...
void CCyberneticsCore::BuildNuclearSilo()
{
